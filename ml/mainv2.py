...
def get_coldstart_prompt_from_events(events: list[dict], mid_to_data) -> str:
    if not events:
        return ""
    prompt = "USER HISTORY:\n"
    for ev in events:
        ...
        prompt += mid_to_data[ev["mid"]] + "\n"
    return prompt


def get_history_prompt_from_events(events: list[dict], mid_to_data) -> str:
    if not events:
        return ""
    ...
    for epoch in range(num_epochs):
        ...
        for step in tqdm(range(num_steps), desc=f"epoch {epoch}"):
            ...
            if uid not in user_logs:
                ...
                cs_labels = [int(r) for r in cs.labels]
                user_logs[uid] = {
                    "cold_start_mids": cs_mids,
                    "cold_start_labels": cs_labels,
                    "events": [
                        {"mid": m, "reward": r}
                        for m, r in zip(cs_mids, cs_labels)
                    ],
                    "interactions": [],
                }

            events = user_logs[uid]["events"]
            prompt = (
                get_coldstart_prompt_from_events(events[:CS_HALF], mid_to_data)
                + get_history_prompt_from_events(events[CS_HALF:], mid_to_data)
            ...
            if chosen_mid is not None:
                events.append({"mid": chosen_mid, "reward": reward})
